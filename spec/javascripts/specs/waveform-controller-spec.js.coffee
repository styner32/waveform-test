describe "WaveformController", ->
  ...
  describe 'render', ->
    it 'appends waveform div', ->
      ...
      console.log waveformController.$frame
      expect($container.find('.frame .waveform').length).toBe(1)

    it 'renders waveform in canvas', ->
      waveformController = new WT.WaveformController($container, waveformData)
      waveformController.render()
      expect($container.find('.frame .waveform canvas').length).toBe(1)

    it 'sets length to length of waveform data', ->
      waveformController = new WT.WaveformController($container, waveformData)
      waveformController.render()
      expect($container.find('.frame .waveform').css('width')).toBe("5px")
    ...
    context 'data size is bigger than 1024', ->
      ...
      it 'sets length to length of waveform data', ->
        waveformController.render()
        expect($container.find('.frame .waveform').css('width')).toBe("3000px")

      it 'splites waveform not to exceed maximum length for each canvas', ->
        waveformController.render()
        expect($container.find('.frame .waveform canvas').length).toBe(3)
